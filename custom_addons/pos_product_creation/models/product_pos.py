...
class ProductFromPos(models.Model):
    ...
    def create_product_pos(self, vals):
        ...
        if vals.get('type') == 'Stockable':
            type = 'product'
        elif vals.get('type') == 'Consumible':
            type = 'consu'
        elif vals.get('type') == 'Servicio':
            type = 'service'
        category = self.env['product.category'].search([('id', '=', vals.get('category'))], limit=1)
        uom_id = self.env['product.uom'].search([('id', '=', vals.get('unit'))], limit=1)
        new_vals = {
            'name': vals.get('name'),
